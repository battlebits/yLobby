		...
		if (Bukkit.getOnlinePlayers().length > 0) {
			for (Player p : Bukkit.getOnlinePlayers()) {
				if ((p.getLocation().clone().subtract(0, p.getLocation().getY(), 0)
						.distance(yLobbyPlugin.getyLobby().getLocationManager().getSpawnLocation().clone().subtract(0, 14, 0)) >= 115
						|| p.getLocation().getY() >= 220 || (p.getLocation().getY() <= 10)) && p.getGameMode() != GameMode.CREATIVE) {
