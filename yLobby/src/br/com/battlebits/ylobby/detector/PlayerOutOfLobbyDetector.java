	...
	public void detect() {
		if (Bukkit.getOnlinePlayers().size() > 0) {
			for (Player p : Bukkit.getOnlinePlayers()) {
				if ((p.getLocation().distance(LobbyMain.getInstance().getLocationManager().getSpawnLocation()) >= 120
						|| p.getLocation().getY() >= 220 || (p.getLocation().getY() <= 5))
						&& p.getGameMode() != GameMode.CREATIVE) {
					if ((p.getLocation().clone().subtract(0, p.getLocation().getY(), 0)
							.distance(LobbyMain.getInstance().getLocationManager().getSpawnLocation().clone()
									.subtract(0, 14, 0)) >= 115
